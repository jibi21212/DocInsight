...
type Store interface {
	...
	ListDocuments(ctx context.Context, page, pageSize int, status *string) ([]model.Document, int, error)
	...
	DeleteDocument(ctx context.Context, id uuid.UUID) (string, error) // returns file_path
	...
	// Search
	MatchEmbeddings(ctx context.Context, queryEmb []float32, threshold float64, limit int, docIDs []uuid.UUID) ([]model.SearchResult, error)
	KeywordSearch(ctx context.Context, queryText string, limit int, docIDs []uuid.UUID) ([]model.SearchResult, error)
	HybridSearch(ctx context.Context, queryEmb []float32, queryText string, threshold float64, limit int, docIDs []uuid.UUID) ([]model.SearchResult, error)
